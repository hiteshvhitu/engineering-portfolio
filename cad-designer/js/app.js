...
document.addEventListener('DOMContentLoaded', () => {
                        ...
                        if (isVideo) {
                            return `
                                <div class="modal-gallery-item" style="position: relative; width: 100%; aspect-ratio: 16/9; background: #0a0e17; display: block; padding: 0; overflow: hidden; border-radius: 8px;">
                                    ...
                                    <div class="video-thumbnail-overlay" onclick="openVideoModal('${item}', '${caption.replace(/'/g, "\\'")}')" style="position: absolute; inset: 0; z-index: 10; cursor: pointer; transition: background 0.3s; background: transparent;" onmouseover="this.style.background='rgba(56,189,248,0.1)'" onmouseout="this.style.background='transparent'">
                                    </div>
                                </div>
                            `;
                        } else {
    ...
    const videoCaptionOverlay = document.getElementById('video-caption-overlay');

    window.openVideoModal = function (url, caption) {
        if (caption && caption.trim() !== "") {
            videoCaptionOverlay.textContent = caption;
            videoCaptionOverlay.style.display = 'block';
        } else {
            videoCaptionOverlay.style.display = 'none';
        }

        videoModalBody.innerHTML = `<iframe src="${url}" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; border: none;" allow="autoplay; fullscreen" allowfullscreen></iframe>`;
    ...
    function closeVideoModal() {
        if (videoModal) {
            ...
            if (videoModalBody) videoModalBody.innerHTML = ''; // Stop video playback
        }
    }

    if (videoModalClose) videoModalClose.addEventListener('click', closeVideoModal);
    if (videoModal) videoModal.addEventListener('click', (e) => { if (e.target === videoModal) closeVideoModal(); });
    document.addEventListener('keydown', (e) => { if (e.key === 'Escape' && videoModal && videoModal.style.display === 'block') closeVideoModal(); });
